# -*- coding: utf-8 -*-
# @File name: plot_general_shutter.py
# @Author: Alicia Garrido-Peña
# @Date:   2023-08-09 19:40:58
...
def plot_mean_error_general(metrics, step_range, path_images, rows=1, df_laser=None):

    fig, axes = plt.subplots(nrows=rows, figsize=(40,20*rows))
    ...
    for i, metric in enumerate(metrics):
        ...
        plot_mean_error(df, "to_off", metric, cut_range, step_range, ax=ax, df_laser=df_laser)
        title = metric if rows>1 else ''
        ax.set_title(title)

    if 'bests_50' in path_images:
        plt.suptitle("Mean changes at different stages of the evolution\n of the action potential dynamics for N=%d"%df[df['file'].str.contains('exp1_')].groupby('file').ngroups)
    else:
        plt.suptitle(path_images + " N=%d"%df[df['file'].str.contains('exp1_')].groupby('file').ngroups)
    if rows==1:
        # plt.legend(metrics)
        plt.legend(['duration','depolarization slope', 'repolarization slope'],loc='upper left')
    plt.tight_layout()
...
if read:
    ...
    save_path = path + path[path[:-1].rfind('/'):-1]
    print("Reading dataframes from "+ save_path +"%s_shutter_*.pkl"%extension)
...
if df.empty:
    raise Exception("No data found, check file path")
...
discard = ["24-03-2023","07-03-2023_exp5","07-03-2023_exp6_","07-03-2023_exp13_","07-03-2023_exp11_"]
df = df[~df.file.str.contains('|'.join(discard))]
df_controls = df_controls[~df_controls.file.str.contains('|'.join(discard))]
df_laser = df_laser[~df_laser.file.str.contains('|'.join(discard))]
df_recovery = df_recovery[~df_recovery.file.str.contains('|'.join(discard))]
print(df)
...
metrics_selection = ['duration', 'depol_slope', 'repol_slope']

for metric in metrics:
    add_norm_diff(df, metric, df_controls=df_controls, df_recovery=df_recovery, df_laser=df_laser, norm=False, general=True)

plot_mean_error_general(metrics_selection, step_range, path_images, rows=len(metrics_selection), df_laser = df_laser)
plt.ylabel("Mean difference from control")

if save:
    savefig(path, path_images, "general_error_mean_to_off")
    savefig(path, path_images, "general_error_mean_to_off", format='.pdf')


for metric in metrics:
    add_norm_diff(df, metric, df_controls=df_controls, df_recovery=df_recovery, df_laser=df_laser, general=True)


# print(df)


plot_mean_error_general(metrics_selection, step_range, path_images)
plt.ylabel("Normalized mean error")
plt.xlabel("Time from end of illumination to spike peak (ms)")


if save:
    savefig(path, path_images, "general_error_mean_norm_to_off")
    savefig(path, path_images, "general_error_mean_norm_to_off", format='.pdf')
